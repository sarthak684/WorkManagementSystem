...
import { addNotifications, getAllNotifications, getNotifications } from "../../../FirebaseAction/Notifications";
...
const Home = ({ navigation }) => {
    ...
    useEffect(()=>{
        ...
        const fetchTasks = async () =>
        {
            ...
            console.log(filteredData.length,'length of intial data from firebase-----')
            const temp2=filteredData.length;
            //settodaytask(temp2)
        ...
        {
            const AllNot = await getAllNotifications();
            const AllNotKeys = Object.keys(AllNot);
            if (AllNotKeys.includes(todayDateString) )
            {
            ...
            {
                addNotifications(todayDateString,{})
            }
    ...
    useEffect(()=>{
        ...
        console.log(filteredData.length,'length of filtered data')
        console.log('Home - UseEffect Called-------++++++++',true);
    ...
    const fetchData = useCallback(() => {
        ...
        console.log('Home - UseEffect Called-------++++++++',true);
        console.log(global.UserPhoto,'link for image=====')
        setImage(global.UserPhoto)
        setF2(true);
        const fetchTasks = async () => {
            const DataTasks = await getTasks();
            // console.log('------------------------------------------------------DataTasks:',DataTasks);
