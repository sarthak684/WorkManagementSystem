...
const CreateTask = () => {
    ...
    const TaskComplete = async () => 
    {
        if(taskname.length===0){
            ...
            Alert.alert('TaskName cannot be null')
        }
        else if(taskname.charAt(0)===' '){
            Alert.alert('Task Name cannot start with space')
        }
        else if(description.length===0){
            Alert.alert('Please add some Description about Task')
        }
        else{
        ...
        <ScrollView contentContainerStyle={{ flexGrow: 1, backgroundColor: white, paddingBottom: '7%' }}>
            ...
            style={[styles.textinput, { color: grey }]}
             onChangeText={(input) => setTaskname(input)}
              maxLength={20} ref={inputRef} value={taskname} key={key}/>
            ...
            <TextInput style={styles.textinput2} placeholderTextColor={grey} placeholder='Enter Short Description about your task' multiline={true} onChangeText={(input) => setDescription(input)} maxLength={400} ref={inputRef1}/>
